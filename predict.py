...
class ActivityPredictor:
    def __init__(self, host='192.168.1.6', port=8080, buffer_size=128):
        self.running = True
        ...
        self.stride = 8
        self.min_confidence = 0.6  # Tăng ngưỡng để yêu cầu độ tin cậy cao hơn
        self.prediction_threshold = 0.6  # Tăng ngưỡng để cải thiện độ ổn định
        self.smoothing_window = 7  # Tăng window để làm mượt dự đoán
    ...
    def _init_socket(self):
        ...
        for port in range(8080, 8090):
            try:
                ...
                print(f"Server đang lắng nghe tại {self.host}:{port}")
                break
    ...
    def _load_scaler(self):
        try:
            ...
            print("Không tìm thấy scaler đã lưu, sẽ fit scaler mới với dữ liệu đầu vào")
            self.scaler = StandardScaler()
    ...
    def receive_data(self):
        print("Đang chờ kết nối từ ESP32...")
            ...
            try:
                ...
                while '\n' in buffer:
                    ...
                    try:
                        ...
                        print(f"Raw sensor data: {data_point}")  # Debug dữ liệu thô
                        ...
                        if data_points_collected >= self.samples_needed and \
                           (current_time - collection_start_time) >= self.prediction_window:
    ...
    def predict_activity(self):
        scaler_buffer = []
        need_fit_scaler = hasattr(self, 'need_fit_scaler') and self.need_fit_scaler
        batch_size = 4
        ...
        while self.running:
            try:
                if not self.data_queue.empty():
                    ...
                    if need_fit_scaler:
                        scaler_buffer.extend(data)
                        if len(scaler_buffer) >= 1000:
                            print("Fitting scaler...")
                            self.scaler.fit(np.array(scaler_buffer))
                            need_fit_scaler = False
                            np.savez('processed_data/scaler_params.npz',
                                   mean=self.scaler.mean_,
                                   scale=self.scaler.scale_)
                            print("Đã fit và lưu scaler")
                        continue
                    ...
                    if len(self.sequence_buffer) >= self.sequence_length:
                        try:
                            ...
                            for i in range(0, len(self.sequence_buffer) - self.sequence_length + 1, self.stride):
                                ...
                                # Thêm đặc trưng: magnitude của gia tốc
                                accel_magnitude = np.sqrt(np.sum(seq[:, :3]**2, axis=1, keepdims=True))
                                gyro_magnitude = np.sqrt(np.sum(seq[:, 3:]**2, axis=1, keepdims=True))
                                seq = np.concatenate([seq, accel_magnitude, gyro_magnitude], axis=1)
                                sequences.append(seq)
                            
                            if sequences:
                                sequences = np.array(sequences)
                                sequences_scaled = self.scaler.transform(
                                    sequences.reshape(-1, 8)  # 6 gốc + 2 magnitude
                                ).reshape(-1, self.sequence_length, 8)
                                print(f"Scaled data sample: {sequences_scaled[-1][0]}")  # Debug dữ liệu đã chuẩn hóa
                                ...
                                for i in range(0, len(sequences_scaled), batch_size):
                                    ...
                                    for pred, weight in zip(preds, batch_weights):
                                        ...
                                        print(f"Class probabilities: {dict(zip(self.activities, pred))}")  # Debug xác suất
